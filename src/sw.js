...
workbox.routing.registerRoute(
  ...
  new RegExp('https://vogelgids-register.netlify.app/.*\.js$'),
  workbox.strategies.cacheFirst({
    cacheName: 'vgr-js-cache',
    plugins: [
...
workbox.routing.registerRoute(
  // match only with assets on the assets domain
  new RegExp('https://vogelgids-register.netlify.app/.*\.css$'),
  workbox.strategies.cacheFirst({
    cacheName: 'vgr-css-cache',
    plugins: [
      new workbox.expiration.Plugin({
        // 28 days cache before expiration
        maxAgeSeconds: 24 * 60 * 60 * 28,
...
workbox.routing.registerRoute(
  // match only with assets on the assets domain
  new RegExp('https://unpkg.com/open-props.*'),
  workbox.strategies.cacheFirst({
    cacheName: 'vgr-css-cache',
    plugins: [
      new workbox.expiration.Plugin({
...
workbox.routing.registerRoute(
  // match only with assets on the assets domain
  new RegExp('https://vogelgids-register.netlify.app/.*\.woff2$'),
  workbox.strategies.cacheFirst({
    cacheName: 'vgr-font-cache',
    plugins: [
      new workbox.expiration.Plugin({
        // 28 days cache before expiration
        maxAgeSeconds: 24 * 60 * 60 * 28,
